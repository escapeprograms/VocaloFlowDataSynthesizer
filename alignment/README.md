...
- **Speed optimizations**:
  ...
  - Sleep reduced from 1.0s to 0.2s per render.
...
- `_compute_dtw_warp_path(prior_audio_path, target_audio_path, sr, cached_target_mel=None)`: Computes DTW warp path. Accepts optional pre-computed target mel to avoid redundant reloading.
...
- Imports: `utils.vocoders`, `stages.synthesizePrior` (lazy — both `generate_prior_from_notes` and `rerender_prior_with_adjusted_durations`).
